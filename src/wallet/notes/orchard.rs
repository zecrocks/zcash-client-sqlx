//! Orchard note storage and retrieval operations.
...
/// Parses an Orchard note from database row data.
///
/// Returns `Ok(None)` if the note cannot be spent (missing UFVK or key scope).
#[cfg(feature = "postgres")]
#[allow(clippy::too_many_arguments)]
pub fn parse_orchard_note<P: consensus::Parameters>(
    params: &P,
    note_id: i64,
    txid_bytes: &[u8],
    action_index: i32,
    ...
    // Parse the note ID
    let note_id = ReceivedNoteId::new(ShieldedProtocol::Orchard, note_id);
    ...
    let diversifier = {
        ...
        orchard::keys::Diversifier::from_bytes(tmp)
    };
    ...
    // Parse rho
    let rho = {
        let rho_array: [u8; 32] = rho_bytes
            .try_into()
            .map_err(|_| SqlxClientError::Encoding("Invalid rho length".to_string()))?;
        Rho::from_bytes(&rho_array)
            .into_option()
            .ok_or_else(|| SqlxClientError::Encoding("Invalid rho value".to_string()))?
    };
    ...
    let rseed = {
        let rseed_array: [u8; 32] = rseed_bytes
            .try_into()
            .map_err(|_| SqlxClientError::Encoding("Invalid rseed length".to_string()))?;
        RandomSeed::from_bytes(rseed_array, &rho)
            .into_option()
            .ok_or_else(|| SqlxClientError::Encoding("Invalid rseed value".to_string()))?
    };
    ...
    let orchard_fvk = ufvk
        .orchard()
        .ok_or_else(|| SqlxClientError::Encoding("UFVK has no Orchard component".to_string()))?;

    let recipient = orchard_fvk.address(diversifier, spending_key_scope);

    // Construct the Orchard note
    let note = Note::from_parts(recipient, NoteValue::from_raw(note_value), rho, rseed)
        .into_option()
        .ok_or_else(|| SqlxClientError::Encoding("Failed to construct Orchard note".to_string()))?;
...
/// Retrieves a specific spendable Orchard note by transaction ID and action index.
///
...
#[cfg(feature = "postgres")]
pub async fn get_spendable_orchard_note<P: consensus::Parameters>(
    pool: &Pool,
    params: &P,
    wallet_id: WalletId,
    txid: &TxId,
    ...
    let query = r#"
        ...
        GROUP BY rn.id, t.txid, rn.action_index, rn.diversifier, rn.value, rn.rho, rn.rseed,
...
/// Selects spendable Orchard notes to meet a target value.
///
...
#[cfg(feature = "postgres")]
#[allow(clippy::too_many_arguments)]
pub async fn select_spendable_orchard_notes<P: consensus::Parameters>(
    pool: &Pool,
    params: &P,
    wallet_id: WalletId,
    account: AccountUuid,
    ...
    // Uses target_height ($8) for spent notes exclusion (tx_unexpired_condition)
    //
    ...
    let query = r#"
        ...
        SELECT id, txid, action_index, diversifier, value, rho, rseed, commitment_tree_position,
    ...
    ) in rows
    {
        ...
        if let Some(note) = parse_orchard_note(
            params,
            note_id,
            &txid_bytes,
            action_index,
...
/// Selects all unspent Orchard notes for an account.
///
/// This is used for operations that need all notes, regardless of confirmation status.
/// Unlike `select_spendable_orchard_notes`, this does NOT filter by anchor height.
///
/// - `target_height` is used for the spent notes exclusion clause (tx_unexpired_condition)
/// - `anchor_height` is accepted for API compatibility but NOT used for filtering
#[cfg(feature = "postgres")]
pub async fn select_unspent_orchard_notes<P: consensus::Parameters>(
    pool: &Pool,
    params: &P,
    wallet_id: WalletId,
    account: AccountUuid,
    ...
    // - target_height ($5) is used for spent notes exclusion (tx_unexpired_condition)
    let query = r#"
        SELECT
            rn.id,
            t.txid,
...
pub async fn get_orchard_nullifiers(
    ...
    for (account_uuid, nf_bytes) in rows {
        let nf =
            Nullifier::from_bytes(<&[u8; 32]>::try_from(nf_bytes.as_slice()).map_err(|_| {
                SqlxClientError::Encoding(
                    "unable to parse Orchard nullifier: expected 32 bytes".to_string(),
                )
            })?)
            .into_option()
            .ok_or(SqlxClientError::Encoding(
...
/// Detects which accounts are affected by the given Orchard nullifiers being spent.
///
...
pub async fn detect_spending_accounts<'a>(
    ...
    let nf_bytes: Vec<Vec<u8>> = nfs.map(|nf| nf.to_bytes().to_vec()).collect();
...
pub(crate) async fn mark_orchard_note_spent(
    pool: &Pool,
    wallet_id: WalletId,
    tx_ref: TxRef,
    nf: &Nullifier,
) -> Result<bool, SqlxClientError> {
    // Insert into orchard_received_note_spends, selecting the note ID based on the nullifier
    let query = r#"
...
impl<AccountId: Copy> ReceivedOrchardOutput
    for zcash_client_backend::wallet::WalletOrchardOutput<AccountId>
{
    type AccountId = AccountId;

    fn index(&self) -> usize {
        zcash_client_backend::wallet::WalletOrchardOutput::index(self)
    }
    fn account_id(&self) -> Self::AccountId {
        *zcash_client_backend::wallet::WalletOrchardOutput::account_id(self)
    }
    fn note(&self) -> &Note {
        zcash_client_backend::wallet::WalletOrchardOutput::note(self)
    }
    fn memo(&self) -> Option<&MemoBytes> {
        None
    }
    fn is_change(&self) -> bool {
        zcash_client_backend::wallet::WalletOrchardOutput::is_change(self)
    }
    fn nullifier(&self) -> Option<&Nullifier> {
        self.nf()
    }
    fn note_commitment_tree_position(&self) -> Option<Position> {
        Some(zcash_client_backend::wallet::WalletOrchardOutput::note_commitment_tree_position(self))
    }
    fn recipient_key_scope(&self) -> Option<Scope> {
        zcash_client_backend::wallet::WalletOrchardOutput::recipient_key_scope(self)
    }
}

impl<AccountId: Copy> ReceivedOrchardOutput
    for zcash_client_backend::DecryptedOutput<Note, AccountId>
{
    type AccountId = AccountId;
    ...
    P: consensus::Parameters,
{
    ...
    if output.recipient_key_scope() != Some(Scope::Internal) {
        ...
        // Get the Orchard IVK to decrypt the diversifier index
        let orchard_ivk = match uivk.orchard() {
            Some(ivk) => ivk,
            None => return Ok(None), // No Orchard capability
        };

        // Decrypt the diversifier index from the note's recipient address
        // Note: Orchard IVK's diversifier_index takes an Address, not a Diversifier
        let to = output.note().recipient();
        let diversifier_index = orchard_ivk.diversifier_index(&to).ok_or_else(|| {
            SqlxClientError::Encoding(
                "Could not derive diversifier index from note recipient".to_string(),
            )
        })?;
    ...
    P: consensus::Parameters,
{
    ...
    let query = r#"
        ...
        ON CONFLICT (wallet_id, tx_id, action_index) DO UPDATE
        SET account_id = EXCLUDED.account_id,
            ...
            is_change = orchard_received_notes.is_change OR EXCLUDED.is_change,
    ...
    let received_note_id: (i64,) = sqlx_core::query_as::query_as(query)
        ...
        .bind(diversifier.as_array())
        .bind(output.note().value().inner() as i64)
        .bind(&output.note().rho().to_bytes()[..])
        .bind(&rseed.as_bytes()[..])
        .bind(memo_bytes)
        .bind(output.nullifier().map(|nf| nf.to_bytes().to_vec()))
